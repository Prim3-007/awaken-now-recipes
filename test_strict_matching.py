...
def clean_and_tokenize(text):
    # Lowercase and replace non-alphanumeric (except spaces) with space
...
def is_phrase_match(allowed_phrase, ing_phrase):
    ...
    # Check if allowed_words is a sub-sequence of ing_words
    # e.g., allowed: "bell pepper", ingredient: "colored bell peppers"
    # allowed_words = ['bell', 'pepper'], ing_words = ['colored', 'bell', 'peppers']
    for i in range(len(ing_words) - len(allowed_words) + 1):
        ...
        for j in range(len(allowed_words)):
            if not is_word_match(allowed_words[j], ing_words[i+j]):
                ...
                break
        if match:
            # Check that it's not matching a generic word only, but let's assume if it matches the whole phrase it's good
            return True

    # Check if ing_words is a sub-sequence of allowed_words
    # e.g., allowed: "coriander leaves", ingredient: "coriander"
    # ing_words = ['coriander'], allowed_words = ['coriander', 'leaves']
    # But wait, we must avoid matching generic words like "leaves", "water", "powder", "oil", "salt", "juice", "seeds"
    # If the matched ingredient is just a generic word, we reject it.
    ...
    for i in range(len(allowed_words) - len(ing_words) + 1):
        ...
        for j in range(len(ing_words)):
            if not is_word_match(ing_words[j], allowed_words[i+j]):
                ...
                break
        if match:
            # Ensure at least one word in ing_words is NOT a generic word
...
def validate_recipe_strict(recipe, season_key, seasons_data):
    season = seasons_data.get(season_key)
    if not season:
        return False, recipe["ingredients"]

    allowed_items = season["allowedProduce"] + season["evergreenStaples"]
    unmatched = []
    matched_details = []
...
def main():
    ...
    print("=== STRICT MATCHING AUDIT ===")
    for season_key, season_info in seasons.items():
        print(f"\nSeason: {season_info['displayName']} ({season_key.upper()})")
        matched = []
        for recipe in recipes:
            is_match, unmatched, details = validate_recipe_strict(recipe, season_key, seasons)
            if is_match:
